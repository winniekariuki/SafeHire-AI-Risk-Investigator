...
export function PlatformAsk({ enabled }: { enabled: boolean }) {
  ...
  }, [question, enabled, getToken]);

  return (
    <SectionCard title="Ask the platform">
      {!enabled ? (
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Load workers first to ask cross-worker questions from the indexed evidence.
        </p>
      ) : (
        <>
          <p className="mb-3 text-sm text-zinc-600 dark:text-zinc-400">
            Questions run across all workers with ingested documents in Supabase—no need to run a
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div className="min-w-0 flex-1">
              ...
              <textarea
                ...
                disabled={loading}
                placeholder="e.g. Which workers mention childcare experience?"
            ...
            <button
              ...
              onClick={() => void submit()}
              className="inline-flex h-10 shrink-0 items-center justify-center rounded-lg bg-emerald-700 px-5 text-sm font-medium text-white transition hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-emerald-600 dark:hover:bg-emerald-500"
            >
              {loading ? "Asking…" : "Ask platform"}
            </button>
          </div>
          {error ? (
